...
ThunderDoggy 是一套由 **MaixCAM 视觉端**和 **ESP32 MicroPython 车控端**组成的低速自动火星车工程。系统支持二维码任务、九宫格自动对正、对正后颜色快照和两行自动夹取。

> 前两行姿态与 ID 14 夹爪参数已经写入，但机械臂安全锁默认关闭；必须完成六格受控实机验证后才能解除锁定。
...
- 九块候选的颜色数量不是 `3+3+3` 时，按三列进行颜色投票并硬纠正为三种不同颜色、每色三块；纠正结果仍须通过完整 3×3 几何校验，纠正块在相机画面中以 `!` 标记。
...
- 收到 ESP32 的对正完成快照请求后，丢弃旧稳定窗口，仅用新帧生成显式九格颜色坐标；未收到 ACK 时自动重发。
...
- 抓取队列按 row 0→1、每行 column 0→2 扫描；二维码颜色书写顺序不改变格位优先级。
...
- PS2 `START` 切换完整自动模式，`L3` 切换对位夹取模式；两种模式互斥。
- `×` 随时取消自动任务并失能驱动电机；PS2 丢线也会停止车辆。
- 机械臂动作框架为：悬停 → 夹取 → 闭爪 → 悬停 → 料斗 → 开爪；下一轮不回初始位。
...
@STREAM_STOP
@QR_ACK,seq
@GRID_SNAPSHOT_REQ,request_id
@GRID_COLORS_ACK,request_id,vision_seq
```

车辆对正成功后保持停车和视觉角，ESP32 请求相机重新采样。相机连续稳定后返回：

```text
@GRID_COLORS,request_id,vision_seq,9,
0,0,color,0,1,color,...,2,2,color
```

ESP32 只接受当前请求 ID 的完整九格快照；等待超时、坐标重复或任务超过已标定两行容量时保持停车并进入故障。
...
4. 相机到位后才开始等待九宫格并自动对正；对正完成后重新采集颜色快照，再按行优先顺序夹取目标。
...
| 右摇杆左右 | 相机舵机旋转 |
| START | 启动/取消完整自动模式 |
| L3 | 无二维码时启动白纸对正；有缓存任务时启动自动对位夹取；再次按下取消/确认退出 |
| × | 取消自动任务、急停并失能驱动电机 |
| △ | 重新使能驱动电机 |
...
前两行六格已填入悬停/夹取姿态，第 2 行三格保持 `None`。自动抓取安全锁仍关闭。
...
3. 每格分别记录悬停和夹取两个姿态。
4. 当前只验证 `(0,0)～(1,2)` 六格；第 2 行需要第三个同色物块时再补充。
5. 保持 `ARM_GRID_CALIBRATED = False`，先逐格执行受控测试；六格姿态、夹爪和料斗路径全部确认后才能改为 `True`。

数据格式：

```python
ARM_GRID_POSES = {
    (0, 0): {"hover": (Roll, Pitch1, Pitch2, Pitch3),
             "grab": (Roll, Pitch1, Pitch2, Pitch3)},
    # ...
    (2, 2): (Roll, Pitch1, Pitch2, Pitch3),
}
```
...
`R3` 当前只把关节姿态和白纸高度打印到串口，不会自动修改或保存 `robot_config.py`。这是有意保留的安全限制：程序并不知道本次记录属于哪个格子，必须由操作者确认后手动填写，避免姿态写错位置。
...
- 受控验证前两行六组悬停/夹取姿态、ID 14 夹爪和料斗路径，再解除安全锁。
- 如任务需要同色第三个物块，再补充第 2 行三格姿态。
