...
- PS2 `START` 切换自动模式，`×` 随时取消自动任务并失能驱动电机；PS2 丢线也会停止车辆。
...
再次按 `START` 可取消自动模式；发生故障后应先按 `×`，检查原因再按 `△` 重新使能。
...
2. 保持自动模式关闭，使用方向键、`L1/L2` 和 `R1/R2` 手动移动机械臂。
3. 到达某格上方安全悬停位置，按 `R3`。
4. 记录终端输出的四关节角度作为 `hover`。
5. 缓慢移动到顶端舵机刚好轻触物块的位置，再按 `R3`。
6. 记录为 `touch`。
