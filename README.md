...
3. 缓慢移动到该格对应的夹取位置，按 `R3`，记录终端输出的四关节角度。
4. 对九个格子重复以上步骤，共采集 9 组姿态，并逐项填入 `ARM_GRID_POSES`。
5. 保持 `ARM_GRID_CALIBRATED = False`，先逐格执行受控测试；全部姿态确认没有越界、碰撞或反向运动后，才能把它改为 `True`。

数据格式：

```python
ARM_GRID_POSES = {
    (0, 0): (Roll, Pitch1, Pitch2, Pitch3),
    # ...
